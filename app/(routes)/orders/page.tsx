...
export default function OrdersPage() {
  ...
  const getTotalItems = () => {
    return orders.reduce((total, order) => total + order.orderItems.length, 0)
  }
  ...
  return (
    <Container>
      <div className="py-8 md:py-12 lg:py-16">
        ...
        {/* Stats Section - Only show when orders exist */}
          ...
          {isLoading ? (
            ...
            >
              ...
              <p className="text-lg text-center max-w-md mb-8 leading-relaxed">
                It looks like you haven&apos;t placed any orders with us yet. Start exploring our amazing products and make
                your first purchase!
              </p>
              <Button
                asChild
                size="lg"
                className="bg-primary text-primary-foreground hover:bg-primary/90 shadow-lg shadow-primary/20 transition-all duration-200 hover:scale-105"
              >
                <link href="/">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
                {orders.map((order, index) => {
                  ...
                  return (
                    <motion.div
                      key={order.id}
                      className="border border-border rounded-2xl p-6 shadow-lg bg-card flex flex-col justify-between transition-all duration-300 hover:shadow-xl hover:border-primary/30 group"
                        ...
                        <div className="space-y-3">
                          ...
                          <ul className="space-y-3">
                            {order.orderItems.map((item) => (
                              <li
                                key={item.id}
                                className="flex items-center gap-3 p-3 bg-muted/20 rounded-lg group-hover:bg-muted/30 transition-colors"
                              >
                                ...
                                {item.product && !order.isPaid &&(
                                  <div className="flex-shrink-0">
                                    <DownloadButton storeId={item.storeId} productId={item.productId} disabled={!order.isPaid}/>
                                  </div>
                                )}
                              </li>
                            ))}
