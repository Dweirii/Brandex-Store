...
export default async function PackagingLandingPage() {
  ...
  const firstImage = (p: P) => getDisplayImageUrl(p.images.find((i) => i?.url)?.url, Number(p.price) === 0)

  const withImage = newest.products.filter(hasImage)
  const heroImages = withImage.slice(0, 10).map((p) => ({ url: firstImage(p), alt: p.name }))
  ...
  const topSubcats = top10.map((s, i) => {
    ...
    return { ...s, image: p ? firstImage(p) : null }
  })
  ...
  return (
    <main className="min-h-screen bg-[#FAFAFA] pb-16 dark:bg-background">
      ...
      <CategoryLandingHero
        ...
        subtitle="From coffee bags to cosmetics, every Brandex packaging template ships print-ready with accurate dielines and editable PSD, AI & EPS files. Pick a design, drop in your brand, and send it straight to print."
        browseHref={BROWSE_HREF}
        browseLabel={`Browse ${total.toLocaleString()} templates`}
        secondaryHref="#categories"
        secondaryLabel="Explore categories"
        badges={["Editable PSD · AI · EPS", "Print-ready dielines", "Commercial license"]}
        stats={[
          { value: total.toLocaleString(), label: "Templates" },
          { value: String(subcategories.length), label: "Categories" },
      ...
      {/* Category marquee */}
      <div className="mt-2">
        <CategoryMarquee items={subcategories} hrefBase={BROWSE_HREF} />
      </div>
        ...
        <Reveal>
          <div className="mb-7 flex items-end justify-between gap-4">
            <div>
              ...
              <p className="mt-1.5 text-sm text-muted-foreground">Top {topSubcats.length} of {subcategories.length} categories.</p>
            </div>
            <Link
              href={BROWSE_HREF}
      ...
      {showcase.length > 0 && (
        <section className="mx-auto w-full max-w-[1320px] px-4 pt-16 sm:px-6">
          ...
          <Reveal delay={0.05}>
            <ArchiveCardGrid products={showcase} />
          </Reveal>
        </section>
      )}
          ...
          <div className="grid gap-8 rounded-3xl border border-border/60 bg-background p-8 sm:p-10 lg:grid-cols-[1fr_1.1fr] lg:gap-12">
            <div>
              ...
              <p className="mt-4 text-sm leading-relaxed text-muted-foreground sm:text-base">
                Brandex is a library of {total.toLocaleString()} professionally crafted packaging designs — spanning{" "}
                {subcategories.length} categories from chocolate bars and coffee bags to cosmetics, beverages and
      ...
      <section className="mx-auto w-full max-w-[1320px] px-4 pt-16 sm:px-6">
        <Reveal>
          <div className="rounded-3xl bg-foreground px-6 py-16 text-center sm:px-12 sm:py-20">
            <h2 className="mx-auto max-w-2xl text-3xl font-semibold tracking-tight text-background sm:text-4xl">
              Find your next packaging design
            </h2>
            <p className="mx-auto mt-3 max-w-md text-sm text-background/60 sm:text-base">
              {total.toLocaleString()} editable, print-ready templates across {subcategories.length} categories.
            </p>
            <Link
              href={BROWSE_HREF}
              className="group mt-8 inline-flex h-12 items-center gap-2 rounded-full bg-primary px-8 text-sm font-semibold text-white transition-colors hover:bg-[#009915]"
            >
